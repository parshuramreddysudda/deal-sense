...
// --------------------------------------------------------------------
// Fix 9 — Input sanitisation constants
// --------------------------------------------------------------------
const DEFAULTS = {
    country: 'IN',
...
const GAS_INTENSITY = 0.202; // kgCO2 / kWh
...
function computeApplicability({ country, isEuListed, isFundManager, isPubliclyListed, annualElectricity }) {
    ...
    return {
        ...
        'GHG Scope 1': true,
        'GHG Scope 2': true,
        'Refrigerant Tracking': countryInfo.eu || !!isEuListed, // gated by CSRD
...
app.get('/api/market-data', async (req, res) => {
    await delay(300);
    const country = str('country', req.query.country, Object.keys(COUNTRY_DATA));
...
app.post('/api/analyse', async (req, res) => {
    ...
    // ---- HVAC (new structured list, back-compat with legacy fields) ----
    ...
    const scope2Auto = (annualElectricity * gridIntensity) / 1000; // tonnes CO2
    const scope2 = scope2In != null ? scope2In : scope2Auto;

    // Scope 1 — direct combustion (gas) + refrigerant leakage
    ...
    const refrigerantEmissions = hvacList.reduce((acc, u) => {
        ...
        return acc + (2 * gwp) / 1000; // 2 kg/year default leakage per unit
    }, 0);
    ...
    // ---- envelope + mix adjusted benchmark ----
    ...
    // ================================================================
    // 2. ESG Compliance Score (/100)
    // ================================================================
    let esgScore = 0;

    // Energy-data completeness — 20
    if (annualElectricity > 0 && annualGas > 0) esgScore += 20;
    else if (annualElectricity > 0) esgScore += 12;

    // Carbon performance vs benchmark — 25
    if (carbonStatus === 'green') esgScore += 25;
    else if (carbonStatus === 'amber') esgScore += 15;
    else esgScore += 5;

    // Refrigerant tracking — 15 (only counted if CSRD applies, else pass-through)
    ...
    // Framework data quality — 25, penalise "Unknown" refrigerant under CSRD
    let frameworkPoints = 25;
    if (applicability.CSRD && anyUnknown) frameworkPoints -= 8;
    esgScore += frameworkPoints;

    // Water — up to 7, not-assessed if blank
    let waterScore = 0;
    let waterAssessed = false;
    if (waterConsumption != null) {
        waterAssessed = true;
        const waterIntensity = waterConsumption / gla; // kL / sqm / yr
    ...
    // Waste — up to 8, not-assessed if blank
    let wasteScore = 0;
    ...
    if (rateType === 'Floating') {
        ...
        // Fixed — classic refinancing cliff logic
    ...
    // Unknown / R-22 refrigerant tracking
    ...
    // Energy retrofit if above benchmark
    if (intensity > effectiveBenchmark) {
    ...
    // Apply carbon price to every action
    if (carbonPrice > 0) {
    ...
    // ================================================================
    // 5. ESG Compliance Detail — dynamic (Fix 10)
    // ================================================================
    ...
    // Water + Waste "not assessed" rows for transparency
    esgComplianceDetail.push({
        framework: 'Water',
    ...
    // ================================================================
    // 8. Portfolio synthesis (if numBuildings > 1)
    // ================================================================
        ...
        <html>
            <head>
                <style>
                    ...
                    h1 { color: #0a0b0e; }
                    .score { font-size: 26px; font-weight: bold; background: #161b24; color: #fff; padding: 16px 20px; border-radius: 8px; display: inline-block; margin-right: 12px; }
            ...
            <body>
                ...
                <div class="muted">${(data.context && data.context.address) || ''} · ${(data.context && data.context.countryName) || ''}</div>
                <div class="section">
                    <div class="score">Deal Risk: ${data.dealRiskScore ?? 'N/A'}</div>
                    <div class="score">ESG Compliance: ${data.esgComplianceScore ?? 'N/A'}</div>
                    <div class="score">Market Position: ${data.marketPositionScore ?? 'N/A'}</div>
                </div>
                <div class="section"><h2>Narrative</h2><p>${data.narrative || ''}</p></div>
                <div class="section">
                    <h2>Risk Breakdown</h2>
                    <table>
                        <tr><th>Factor</th><th>Finding</th><th>Severity</th><th>Action</th></tr>
                        ${(data.dealRiskBreakdown || []).map(r => `<tr><td>${r.factor}</td><td>${r.finding}</td><td>${r.severity}</td><td>${r.action}</td></tr>`).join('')}
                    </table>
                </div>
                <div class="section">
                    <h2>Regulatory Applicability</h2>
                    <table>
                        <tr><th>Framework</th><th>Status</th><th>Note</th></tr>
                        ${(data.esgComplianceDetail || []).map(r => `<tr><td>${r.framework}</td><td>${r.status}</td><td>${r.note || ''}</td></tr>`).join('')}
                    </table>
                </div>
                <div class="section">
                    <h2>Action Plan</h2>
                    <table>
                        <tr><th>Action</th><th>CO₂ t/yr</th><th>$ /yr</th><th>Cost</th><th>Payback</th></tr>
                        ${(data.actionPlan || []).map(a => `<tr><td>${a.title}</td><td>${a.co2Saving}</td><td>$${(a.dollarSaving||0).toLocaleString()}</td><td>$${(a.cost||0).toLocaleString()}</td><td>${a.payback}y</td></tr>`).join('')}
                    </table>
                </div>
            </body>
        </html>`;
...
app.listen(PORT, () => {
    console.log(`DealSense backend running on port ${PORT}`);
});
